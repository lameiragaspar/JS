function gerarRelatoriodeComprasPorCliente(){
  ...
  function dadosCliente(cliente){
    return cliente.reduce((acc, {nome, compras}) => {

      compras.forEach(( {produto, categoria, valor}) => {
        //console.log(produto);
        acc[nome] ??= {};
        acc[nome][categoria] ??= {};
        acc[nome][categoria].totalGasto = (acc[nome][categoria].totalGasto ?? 0 ) + valor;
        acc[nome][categoria].produtos ??= [];
        acc[nome][categoria].produtos.push(produto);
      })
  ...
  function gerarSugestoes(comprasporCliente, produtosNormalizados){
    const clientes = Object.keys(comprasporCliente)
